...
warning('KroneckerBio:OutOfDateFunction', 'MapParameterSpace is out of data and needs to be fixed')
...
opts.LowerBound = fixBounds(opts.LowerBound, opts.UseParams, opts.UseSeeds, opts.UseModelSeeds);
opts.UpperBound = fixBounds(opts.UpperBound, opts.UseParams, opts.UseSeeds, opts.UseModelSeeds);
        ...
        for i = 1:nSam
            for j = 1:nSam
                ...
                if strcmpi(opts.EnumSource, 'FIM') || strcmpi(opts.EnumSource, 'hessian')
                    T0 = [m.k(opts.UseParams); m.x0(opts.UseSeeds)];
                    T = [knew(opts.UseParams); x0new(opts.UseSeeds)];
                    if opts.Normalized
                        % Use log parameters
                        deltaT = log(T0 ./ T);
                    else
                    ...
                    if strcmp(opts.EnumSource, 'FIM')
                        Gs(iPair, i, j) = G0 + deltaT.' * F * deltaT;
                    else
                        Gs(iPair, i, j) = G0 + 0.5 * deltaT.' * F * deltaT;
                    end
                else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function val = optFun(l)
        ...
        newK = m.k;
        newIC = m.x0;
        newP = [m.k(opts.UseParams); m.x0(opts.UseSeeds)];
        if opts.Normalized
        ...
        newIC(opts.UseSeeds) = newP(nTk + (1:nTx));

function [] = install_STB_Kronecker
%
...
checkExists = @(mod) exist(fullfile(sun,'src',mod),'dir') > 0;

cvm_ok = checkExists('cvodes');
idm_ok = checkExists('idas');
kim_ok = checkExists('kinsol');
...
% Install in directory containing the SUNDIALS directory, which should be
% the External directory of KroneckerBio
    where = fullfile(sun,'..');
  if isempty(where)
    go = 0;
    break;
